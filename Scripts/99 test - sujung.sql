...
UPDATE MALL_PDT SET DOG_CATE = 9 WHERE DOG_CATE = 1;
...
SELECT a.*
  FROM (SELECT rownum AS rnum, b.*
  	...
  	from MALL_PDT mp LEFT OUTER JOIN MALL_DOG_CATE mdc ON mp.DOG_CATE = mdc.ID LEFT OUTER JOIN MALL_CAT_CATE mcc ON mp.CAT_CATE = mcc.ID ORDER BY mp.ID desc) b) a
 WHERE a.rnum BETWEEN 1 AND 10
 ORDER BY a.rnum
