...
DELETE FROM JOONGO_IMAGE WHERE id= 11;
SELECT * FROM JOONGO_IMAGE;
DELETE FROM JOONGO_IMAGE ji WHERE sale_id = 12;
DELETE FROM joongo_sale WHERE id = 41;
SELECT * FROM JOONGO_SALE js WHERE id = 41;
SELECT * FROM joongo_image WHERE sale_id = 12;

SELECT mall_mileage_seq.nextval FROM DUAL;
SELECT * FROM MALL_MILEAGE mm ;
SELECT * FROM MALL_DOG_CATE mdc ;
SELECT* FROM MALL_CAT_CATE mcc ;
SELECT * FROM MALL_pdt ;

	SELECT a.*
		  FROM (SELECT rownum AS rnum, b.*
		  		FROM (select mp.ID, DOG_CATE AS DOG_ID, mdc.NAME AS DOG_NAME, CAT_CATE AS CAT_ID, mcc.NAME AS CAT_NAME, mp.NAME, PRICE, CONTENT, SALE_YN, STOCK, IMAGE1, IMAGE2, IMAGE3, DELIVERY_KIND, DELIVERY_CONDITION, DELIVERY_PRICE, REGDATE from MALL_PDT mp LEFT OUTER JOIN MALL_DOG_CATE mdc ON mp.DOG_CATE = mdc.ID LEFT OUTER JOIN MALL_CAT_CATE mcc ON mp.CAT_CATE = mcc.ID
		  		WHERE DOG_CATE != 1 AND SALE_YN = 'Y' ORDER BY mp.ID desc) b) a
...
SELECT * FROM JOONGO_SALE js ;
	  ...
	  FROM (SELECT rownum AS rnum, b.*
	  		FROM (SELECT DISTINCT js.ID, MEM_ID, DOG_CATE, CAT_CATE, TITLE, CONTENT, PRICE, d1.ID AS DONGNE1ID, d1.NAME AS DONGNE1NAME, d2.ID AS DONGNE2ID, d2.NAME AS DONGNE2NAME, BUY_MEM_ID, SALE_STATE, REGDATE, REDATE, HITS, CHAT_COUNT, HEART_COUNT , thum_name
	  FROM JOONGO_SALE js  LEFT JOIN JOONGO_IMAGE ji ON  ji.SALE_ID = js.ID  LEFT JOIN DONGNE1 d1 ON js.DONGNE1_ID = d1.ID LEFT JOIN DONGNE2 d2 ON js.DONGNE2_ID = d2.ID WHERE d1.NAME = '부산광역시' ORDER BY js.id desc) b) a
...
SELECT DISTINCT js.ID, MEM_ID, DOG_CATE, CAT_CATE, TITLE, CONTENT, PRICE, d1.ID AS DONGNE1ID, d1.NAME AS DONGNE1NAME, d2.ID AS DONGNE2ID, d2.NAME AS DONGNE2NAME, BUY_MEM_ID, SALE_STATE, REGDATE, REDATE, HITS, CHAT_COUNT, HEART_COUNT, thum_name
	FROM JOONGO_SALE js LEFT JOIN DONGNE1 d1 ON js.DONGNE1_ID = d1.ID LEFT JOIN DONGNE2 d2 ON js.DONGNE2_ID = d2.ID LEFT JOIN JOONGO_IMAGE ji ON  ji.SALE_ID = js.ID WHERE dog_Cate = 'y' AND cat_cate = 'y' ORDER BY js.id DESC ;
...
SELECT count(a.id)
	  ...
	  where DOG_CATE = 'y' and CAT_CATE = 'y'
	  ORDER BY js.id desc) b) a;
	  
	 SELECT a.*
	  FROM (SELECT rownum AS rnum, b.*
	  		FROM (SELECT DISTINCT js.ID, MEM_ID, DOG_CATE, CAT_CATE, TITLE, CONTENT, PRICE, d1.ID AS DONGNE1ID, d1.NAME AS DONGNE1NAME, d2.ID AS DONGNE2ID, d2.NAME AS DONGNE2NAME, BUY_MEM_ID, SALE_STATE, REGDATE, REDATE, HITS, CHAT_COUNT, HEART_COUNT , thum_name
	  FROM JOONGO_SALE js LEFT JOIN JOONGO_IMAGE ji ON  ji.SALE_ID = js.ID LEFT JOIN DONGNE1 d1 ON js.DONGNE1_ID = d1.ID LEFT JOIN DONGNE2 d2 ON js.DONGNE2_ID = d2.ID WHERE d1.NAME = '서울특별시'  ORDER BY js.id desc) b) a
	 WHERE a.rnum BETWEEN 1 AND 10 AND CAT_CATE ='y' AND dog_cate = 'n'
	 ORDER BY a.rnum;
...
SELECT COUNT(A.ID)
			FROM (SELECT ROWNUM AS RNUM, B.*
				FROM (SELECT ROWNUM, ID, OD_ID, MEM_ID, MILEAGE, CONTENT, REGDATE
		FROM MALL_MILEAGE 
